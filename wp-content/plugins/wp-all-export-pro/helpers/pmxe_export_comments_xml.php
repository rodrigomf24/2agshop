						...
						switch ($exportOptions['cc_type'][$ID]) {
							...
							case 'comment_content':
								$xmlWriter->beginElement($element_name_ns, $element_name, null);
									$xmlWriter->writeCData(apply_filters('pmxe_comment_content', pmxe_filter($comment->comment_content, $fieldSnipped), $comment->comment_ID));
								$xmlWriter->endElement();
								break;
								...
								if ( ! empty($exportOptions['cc_value'][$ID]) ){																		
									...
									if (!empty($cur_meta_values) and is_array($cur_meta_values)){
										foreach ($cur_meta_values as $key => $cur_meta_value) {
											$xmlWriter->beginElement($element_name_ns, $element_name, null);
												$xmlWriter->writeCData(apply_filters('pmxe_custom_field', pmxe_filter(maybe_serialize($cur_meta_value), $fieldSnipped), $exportOptions['cc_value'][$ID], $comment->comment_ID));
											$xmlWriter->endElement();
										}
									}
									...
									if (empty($cur_meta_values)){
										$xmlWriter->beginElement($element_name_ns, $element_name, null);
											$xmlWriter->writeCData(apply_filters('pmxe_custom_field', pmxe_filter('', $fieldSnipped), $exportOptions['cc_value'][$ID], $comment->comment_ID));
										$xmlWriter->endElement();
									}																																																												
								}								
								break;																		
							...
							case 'sql':
								...
								if ( ! empty($exportOptions['cc_sql'][$ID]) ){
									...
									$xmlWriter->beginElement($element_name_ns, $element_name, null);
										$xmlWriter->writeCData(apply_filters('pmxe_sql_field', $val, $element_name, $comment->comment_ID));
									$xmlWriter->endElement();
								}
								break;							
